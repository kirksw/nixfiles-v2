...
You have access to specialized subagents via the Task tool. Use them to produce better results
by matching work to the right agent instead of doing everything yourself.
...
deliverable. `scribe` reads git commits as the changelog and writes/updates session logs,
