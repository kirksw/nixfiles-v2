...
if [[ $ZSH_VERSION == ([1-4]*) ]]; then
    ...
    __starship_get_time() {
        STARSHIP_CAPTURED_TIME=$(/etc/profiles/per-user/kisw/bin/starship time)
    }
else
...
PROMPT='$('/etc/profiles/per-user/kisw/bin/starship' prompt --terminal-width="$COLUMNS" --keymap="${KEYMAP:-}" --status="${STARSHIP_CMD_STATUS:-}" --pipestatus="${STARSHIP_PIPE_STATUS[*]:-}" --cmd-duration="${STARSHIP_DURATION:-}" --jobs="$STARSHIP_JOBS_COUNT")'
RPROMPT='$('/etc/profiles/per-user/kisw/bin/starship' prompt --right --terminal-width="$COLUMNS" --keymap="${KEYMAP:-}" --status="${STARSHIP_CMD_STATUS:-}" --pipestatus="${STARSHIP_PIPE_STATUS[*]:-}" --cmd-duration="${STARSHIP_DURATION:-}" --jobs="$STARSHIP_JOBS_COUNT")'
PROMPT2="$(/etc/profiles/per-user/kisw/bin/starship prompt --continuation)"
